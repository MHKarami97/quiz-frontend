<template>
  <div class="min-h-screen p-6 max-w-lg mx-auto flex flex-col justify-center gap-6 pb-24">
    ...
    <div v-else class="text-center">
      <div class="animate-pulse text-xl font-bold mb-2">در حال جستجوی حریف...</div>
...
onMounted(async () => {
  try {
    categories.value = await apiClient.get<Category[]>('/api/categories')
  } catch (err) {
...
async function startMatchmaking(categoryId: string) {
  ...
  try {
    const result = await apiClient.post<MatchmakeResponse>('/api/game/duel/matchmake', {
      categoryId,
    })
    ...
    pollInterval = setInterval(async () => {
      try {
        const poll = await apiClient.get<MatchmakeResponse>('/api/game/duel/matchmake/poll')
        ...
        if (pollInterval) clearInterval(pollInterval)
        errorMessage.value = err instanceof Error ? err.message : 'خطا در جستجوی حریف'
        isSearching.value = false
      }
    }, 2000)
  } catch (err) {
    errorMessage.value = err instanceof Error ? err.message : 'خطا در شروع جستجو'
    isSearching.value = false
  }
}
